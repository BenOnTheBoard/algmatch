...
class AbstractInstanceGenerator(ABC):
    ...
    def _reset_instance(self):
        ...
        self._sp = {f"s{i}": [] for i in range(1, self._num_students + 1)}

        # project -> [capacity, lecturer, student]
        self._plc = {f"p{i}": [1, "", []] for i in range(1, self._num_projects + 1)}
    ...
    def _add_ties_to_list(self, pref_list, density):
        ...
        tied_list = [[pref_list[0]]]
        for elt in pref_list[1:]:
            tied_list = self._assign_using_density(tied_list, elt, density)
        ...
        return delim.join(
            list(
                map(
                    ...
                    [
                        x if len(x := tuple(map(lambda x: int(x[1:]), p))) > 1 else x[0]
                        for p in L
                    ],
                )
            )
    ...
    def write_instance_to_file(self, filename: str) -> None:
        ...
        with open(filename, "w") as f:
            ...
            for student in self._sp:
                f.write(
                    f"{student[1:]}{delim}{self._tied_list_to_string(self._sp[student], delim)}\n"
                )
            ...
            for lecturer in self._lp:
                f.write(
                    f"{lecturer[1:]}{delim}{self._lp[lecturer][0]}{delim}{self._tied_list_to_string(self._lp[lecturer][2], delim)}\n"
                )
